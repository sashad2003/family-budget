...
import { DICT } from '../data/i18n.js?v=116';
 ...
 */
/**
 * Русское число со словом: 1 покупка, 2 покупки, 5 покупок.
 *
 * Русский требует три формы — они передаются списком: держать их в словаре
 * пришлось бы тремя ключами на каждое слово. Английскому и ивриту хватает
 * двух, и они берутся из словаря: ключ для множественного и он же плюс One
 * для единственного.
 */
export function plural(n, [one, few, many], key) {
  // У английского и иврита форм две; ключ для единственного — тот же плюс One.
  if (getLocale() !== 'ru') return t(n === 1 ? `${key}One` : key, { n });
