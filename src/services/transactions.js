...
import { db } from '../core/firebase.js?v=117';
import { getFamilyId } from '../core/session.js?v=117';
import { defaultCategories } from '../data/categories.js?v=117';
import { countUsage } from './usage.js?v=117';
import { amountsInAllCurrencies } from '../core/money.js?v=117';
import { monthOf } from '../core/dates.js?v=117';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=117');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=117');
