...
import { db } from '../core/firebase.js?v=78';
import { getFamilyId } from '../core/session.js?v=78';
import { defaultCategories } from '../data/categories.js?v=78';
import { amountsInAllCurrencies } from '../core/money.js?v=78';
import { monthOf } from '../core/dates.js?v=78';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=78');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=78');
