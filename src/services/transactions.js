...
import { getFamilyId } from '../core/session.js?v=62';
import { defaultCategories } from '../data/categories.js?v=62';
import { amountsInAllCurrencies } from '../core/money.js?v=62';
import { monthOf } from '../core/dates.js?v=62';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=62');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=62');
