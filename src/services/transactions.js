...
import { db } from '../core/firebase.js?v=45';
import { getFamilyId } from '../core/session.js?v=45';
import { defaultCategories } from '../data/categories.js?v=45';
import { amountsInAllCurrencies } from '../core/money.js?v=45';
import { monthOf } from '../core/dates.js?v=45';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=45');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=45');
