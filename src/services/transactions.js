...
import { db } from '../core/firebase.js?v=105';
import { getFamilyId } from '../core/session.js?v=105';
import { defaultCategories } from '../data/categories.js?v=105';
import { countUsage } from './usage.js?v=105';
import { amountsInAllCurrencies } from '../core/money.js?v=105';
import { monthOf } from '../core/dates.js?v=105';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=105');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=105');
