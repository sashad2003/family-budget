...
import { defaultCategories } from '../data/categories.js?v=42';
import { amountsInAllCurrencies } from '../core/money.js?v=42';
import { monthOf } from '../core/dates.js?v=42';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=42');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=42');
