...
import { defaultCategories } from '../data/categories.js?v=43';
import { amountsInAllCurrencies } from '../core/money.js?v=43';
import { monthOf } from '../core/dates.js?v=43';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=43');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=43');
