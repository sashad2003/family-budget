...
import { db } from '../core/firebase.js?v=61';
import { getFamilyId } from '../core/session.js?v=61';
import { defaultCategories } from '../data/categories.js?v=61';
import { amountsInAllCurrencies } from '../core/money.js?v=61';
import { monthOf } from '../core/dates.js?v=61';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=61');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=61');
