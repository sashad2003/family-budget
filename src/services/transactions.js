...
import { db } from '../core/firebase.js?v=106';
import { getFamilyId } from '../core/session.js?v=106';
import { defaultCategories } from '../data/categories.js?v=106';
import { countUsage } from './usage.js?v=106';
import { amountsInAllCurrencies } from '../core/money.js?v=106';
import { monthOf } from '../core/dates.js?v=106';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=106');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=106');
