...
import { db } from '../core/firebase.js?v=40';
import { getFamilyId } from '../core/session.js?v=40';
import { defaultCategories } from '../data/categories.js?v=40';
import { amountsInAllCurrencies } from '../core/money.js?v=40';
import { monthOf } from '../core/dates.js?v=40';
...
async function shareItemPrices(txId, tx, user) {
  try {
    const { publishPrices } = await import('./prices.js?v=40');
...
export async function deleteTransaction(id, user = null) {
  ...
  if (!user) return;
  try {
    const { removePrices } = await import('./prices.js?v=40');
