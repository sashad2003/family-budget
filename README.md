...
Тело письма бывает двух видов: простые абзацы (экранируются, разбиваются по
пустой строке) или своя разметка — заголовки, картинки, таблицы. Рамка письма
и подпись со ссылкой отписки добавляются в обоих случаях, а текстовая часть
для почтовиков без разметки получается вычищением тегов. Язык лежит в `users/{uid}.locale`: сам по себе он
