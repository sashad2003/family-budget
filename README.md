...
Письма уходят через AhaSend по HTTP API v2 — SMTP не используется. Ключ и id
аккаунта лежат в `config.php`, отправляет `api-proxy.php` действием
`send_mail`; право писать людям проверяется там же по `admin_emails`, а не
кнопкой в интерфейсе.

Прокси шлёт по одному письму на адрес, порциями по пятьдесят: в письме на
несколько получателей каждый видит чужие почты. Ссылка «отписаться» ведёт в
приложение с `?unsubscribe=1` — оно выключает письма у вошедшего.
