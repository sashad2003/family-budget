...
Письмо пишется на одном языке, а на остальные его переводит та же модель, что
разбирает чеки (`translate_mail` в прокси): разметку она сохраняет, переводится
только текст между тегами. Перевод можно править руками — каждому уходит
вариант на его языке.

Письмо пишется в редакторе с двумя видами: «как выглядит» — кнопки заголовка,
списка, ссылки и картинки поверх `contenteditable` — и «код», где та же
разметка правится руками. Библиотеки нет: редактору письма нужно немногое, а
всё это умеет сам браузер. Разметку из-под редактора нормализует
`src/ui/richText.js` — тегам проставляются стили прямо в атрибут, потому что
почтовые программы вырезают внешние стили.

Письмо без тегов приложение разбивает на абзацы и экранирует само: спрашивать
об этом человека незачем. Рамка письма и подпись со ссылкой отписки
добавляются всегда, а текстовая часть для почтовиков без разметки получается
вычищением тегов.

Черновик пишется в `localStorage` на каждой букве, а под именем письмо
сохраняется в `mailTemplates/{id}` и открывается с любого устройства. Правила
дают доступ туда только админу. Язык лежит в `users/{uid}.locale`: сам по себе он
...
### 2а. Правила Firestore

Пуш выкладывает файлы сайта, а правила живут в самом Firebase, и деплой их не
возит. Из-за этого код и боевые правила разъезжались незаметно: коллекция в
коде появлялась, а доступ к ней оставался закрытым.
...
Скрипт ходит в Firebase Rules API служебным ключом — тем самым файлом
`*firebase-adminsdk*.json`, который лежит рядом и не коммитится. Ставить
firebase-tools ради одного запроса незачем.
